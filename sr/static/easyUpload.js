    ...
    this.each(function (index, element) {
      ...
      var easyManager = {
        init: function () {
          ...
          //$html += '<span class="easy_upload_head_btn2 noselect">取消</span>';
        ...
        _renderFile: function (fileArr, target) {
          ...
          function render(file) {
            ...
            $html += file.allow ? sHtml : '<p class="status status6">文件格式不符合规范或大小超出限制</p>';
            $html += '</div>';
            $html += '<div class="easy_upload_btn queue_item-section">';
            $html += file.allow ? '<p class="easy_upload_upbtn btn noselect">上传</p>' : '';
            $html += '<p class="easy_upload_delbtn btn noselect">取消</p>';
            $html += '</div>';
